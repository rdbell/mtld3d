...
impl DeviceInner {
    ...
    /// Submit rendering and a trailing readback together. The surface owns
    /// the destination pages and keeps them borrowed until this call returns.
    pub fn flush_with_readback(&mut self, readback: BlitTextureToBufferParams) -> i32 {
        let fresh = self.fresh_frame();
        let (mut frame, _) = self.stamp_and_swap(fresh, true);
