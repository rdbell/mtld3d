...
pub struct FrameData {
    pub readback: Option<mtld3d_shared::BlitTextureToBufferParams>,
    ops: Vec<Op>,
...
impl EncoderThread {
    ...
    /// submission.
    pub fn mid_frame_submit(&self, frame: FrameData) -> i32 {
    ...
    loop {
        match receiver.recv() {
            ...
            Ok(EncoderMessage::MidFrameSubmit { frame, done }) => {
                ...
                let _ = done.send(enc.last_submit_status);
            }
...
fn finalize_submit(enc: &mut FrameEncoder, frame: &FrameData) -> (SubmitFrameParams, FramePayload) {
    ...
    let params = SubmitFrameParams {
        readback_params_ptr: frame.readback.as_ref().map_or(0, |p| p as *const _ as u64),
